...
function studioDownloadFor(recipeId,batchId){
  ...
  var svg=studioRenderSide(d.front,d.w,d.h,ctx,{idSuffix:'dl'});
  studioRasterize(svg,d.w,d.h,3,function(blob){
    if(!blob){toast('⚠ Could not encode PNG');return;}
    var name=((ctx.batch&&ctx.batch.name)||ctx.recipe.name||'mead').replace(/[^\w-]+/g,'_')+'-label.png';
    var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=name;document.body.appendChild(a);a.click();
    setTimeout(function(){URL.revokeObjectURL(a.href);a.remove();},120);toast('✦ Label saved');
  });
}
