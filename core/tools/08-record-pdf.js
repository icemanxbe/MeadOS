...
function openProductionReport(){
  ...
  var brandName=APP.settings.brewerName||'MeadOS';
  var ccy=APP.settings.currency||'€';
  var batches=(APP.batches||[]).slice().sort(function(a,b){return(b.startDate||'').localeCompare(a.startDate||'');});
  ...
  if(typeof mwIngredientStats==='function'){
    ...
    if(topY||topH){
      perfHtml='<h2>'+(nl?'Beste presteerders':'Best performers')+'</h2><table class="kv-table">'
        ...
        +(topH?row(nl?'Beste honing':'Best honey',topH.key+' · '+topH.avgRating.toFixed(1)+'★ ('+topH.n+')'):'')
        +'</table>';
    }
  }
  ...
  w.document.write('<!DOCTYPE html><html><head><title>'+(nl?'Productierapport':'Production Report')+' · '+xmlEscape(brandName)+'</title>'
    ...
    +'<div style="margin-top:28px;font-size:9.5px;color:#999;text-align:center;font-style:italic;letter-spacing:1px">'+(nl?'Gegenereerd door':'Generated by')+' MeadŌS · '+fmtDate(today())+'</div>'
    +'</body></html>');
  w.document.close();
}
