...
- 📜 **38 built-in recipes** — traditionals, melomels, cysers, metheglins, bochets, braggots, sack & port-style meads, plus 5 **sparkling / bottle-conditioned** meads (champagne-style, cyser, session hydromel, berry, ginger), each with day-by-day step schedules, plus your own custom recipes and reusable templates. Steps carry universal conventions automatically (top-up-to-the-mark water, rack-when-fermentation-is-done, stabilise with sorbate **and** metabisulfite before back-sweetening, "earliest bottling point" not a deadline), and tied recipes cross-link — the aroma-sensitive melomels (forest fruits, strawberry, raspberry, blueberry) each come in **fruit-in-primary** and **fruit-in-secondary** versions that link to each other and explain the trade-off
...
### Recipes
38 built-in recipes (including 5 sparkling / bottle-conditioned meads, and four aroma-sensitive melomels — forest fruits, strawberry, raspberry, blueberry — each in cross-linked fruit-in-primary / fruit-in-secondary versions) with full ingredient lists, day-by-day schedules, nutrient plans, and aging guidance. Each recipe page also rates **every honey in the library** for that specific mead — a great / good / workable / clash badge with a note on what the swap does to *this* recipe, your in-stock honeys pinned to the top — alongside recommended yeast strains and a nutrient strategy. Add your own recipes, save variations as templates, scale any recipe to your vessel size (with a metric/US/imperial readout), brew it on the spot or plan a future batch at that scale, favorite the ones you love, and export to BeerXML or a print-ready PDF.
...
- **Built-in hardening.** Beyond the security headers (CSP, X-Frame-Options, etc.): login is rate-limited per IP with a temporary lockout after repeated failures; state-changing requests are origin-checked (CSRF defence on top of the SameSite session cookie); the optional Home Assistant proxy only forwards a small allowlist of endpoints (sensor reads, history, push notifications — never HA config, admin or service-restart); security-relevant events (logins, setting/token changes, restores, proxy denials) are written to an audit log on stderr with no secrets; the dev test page is LAN-only; and the `Server` header doesn't advertise the Python version.
...
- The only external asset is **Google Fonts** (CSS + font files); Chart.js and the QR library are self-hosted, so charts and the bottle-scanner work offline too. Everything else is self-contained — without internet the app still works, fonts just fall back to system defaults.
- **Uploaded images (label art, photos, brand logo) are stored as plain files under `assets/`** — `assets/labels/`, `assets/photos/`, `assets/brand/` — next to `index.html` (content-addressed, deduplicated, served with immutable caching). Legacy `/labels/<name>` references still resolve after the reorg. Include the `assets/` folder in your backups.
